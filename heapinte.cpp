...
int getLeftChildIndex (int parentIndex){
    if (parentIndex > ((heapsize-1)/2 )) {cerr<<"\n Child Not available for current Node .."; return -1 ;}
...
void Heap::insertNode (int key){
 ...
 if (heapsize == asize)
       cerr<<"\nHeap's is Full, Can not enter "<<key<<"  ...";
else {
      heapsize++;
      data[heapsize - 1] = key;
      siftUp(heapsize - 1);
...
void Heap::siftDown(int nodeIndex){
    int left, right, maxChildIndex ;
    while (nodeIndex < heapsize){
        left = getLeftChildIndex (nodeIndex);
        right = getRightChildIndex (nodeIndex);
        if (left != -1 && right != -1){ maxChildIndex = (data[left] < data [right]) ? right : left; cout<<"\nMaxchild"<<maxChildIndex; }
        else if (left != -1 && right == -1) maxChildIndex = left;
        else if (left == -1 && right != -1) maxChildIndex = right;
  //      else break;
        if ( data[nodeIndex] < data[maxChildIndex]){
        exchange( &data[maxChildIndex], &data[nodeIndex]);
        nodeIndex= maxChildIndex;}
    }
}
 */
void Heap :: siftDown (int nodeIndex) {
    int left=2*nodeIndex+1; //indexoftheleftchild
    while(left <= heapsize)
        {
        if(left<heapsize)
            if(data[left]<data[left+1])
                left=left+1; //indexofthelargerchild
        if(data[nodeIndex]>data[left]) //thesubtreeisalreadyin //aheap
            break;
        else
            {
            exchange(&data[nodeIndex],&data[left]); //Lineswap**
            nodeIndex=left; //gotothesubtreetofurther
//restoretheheap
            left=2*nodeIndex+1;
        }
    }
...
cout<<"\nRemoved Node is :" <<myheap->deQueue()<<endl;
myheap->printHeap();
return 0 ;
}
